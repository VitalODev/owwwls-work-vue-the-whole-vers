<template>
      ...
      <v-flex xs12 sm11 md10 xl8>
        <div class="div-spacer">
          ...
          <div class="text-xs-left pl-3 pb-3">
            <a href="https://github.com/VitalODev/koa-vue-authentication" target="_blank" style="text-decoration: none">
              <v-btn dark>
                source code
  ...
  "scripts": {
    "serve": "babel-node src"
          ...
          <p :style="{ fontSize: computedStyle.fontSizeP }">Now, let's create an <var>src</var> directory with an <var>index.js</var> file inner. This file is the so-called entry point to our application. It's time to install Koa - <kbd>yarn add koa</kbd>. In the <var>index.js</var> - <code>import Koa from 'koa'</code>, and create an instance right away - <code>const app = new Koa()</code>. It's also possible to define common middleware for error handling:</p>
          <code :style="{ fontSize: computedStyle.fontSizeP }" class="px-3">
  ...
  methods: {
    logout () {
      ...
      this.$router.push('/')
          ...
          <p :style="{ fontSize: computedStyle.fontSizeP }">Well, if the saying goes, all roads lead to Rome, then us they lead to the Store:) Return to the Store to implement our latest method in the actions. Well, I'm going to show all the code at once:</p>
          <code :style="{ fontSize: computedStyle.fontSizeP }" class="px-3">
    ...
    beforeEnter: (to, from, next) => {
      Store.dispatch('guard')
        ...
        .catch()
          ...
          <p :style="{ fontSize: computedStyle.fontSizeP }">We use the help of <a href="https://router.vuejs.org/guide/advanced/navigation-guards.html#global-before-guards" target="_blank">Navigation Guard</a>. That's all! 'Reinforced' authentication has been completed. Good luck to you and let an audience of your site be under reliable protection!</p>
        </div>
      </v-flex>
    </v-layout>
  </v-container>
