<template>
  <div ref="player" id="player" @mousedown.prevent>
    <h1 :style="{ fontSize: computedStyle.fontSizeH1 }">Are you not hungry? Ok, look at the movies:</h1>
    ...
    <div ref="controls" id="controls">
      ...
      <span id="sound-controller" class="control-element material-icons" @mouseover="isSoundLine = true" @mouseout="isSoundLine = false">{{ volumeIcon }}
...
export default {
  ...
  methods: {
    ...
    setCurrentTime () {
      ...
      this.currentTimes[this.currentMovie] = this.currentMovieElement.currentTime
      this.timePointersWidth[this.currentMovie] = this.currentMovieElement.currentTime / this.durations[this.currentMovie] * 100 + '%'
    },
    setVolumeAtStart () {
      this.currentMovieElement.volume = this.volume
    },
    ...
    setVolumeFromSoundLine (event) {
      ...
      let volumePointerPosition = (((this.$refs.controls.offsetTop - (event.pageY - 48)) / this.$refs.soundLine.offsetHeight) - 0.01).toFixed(2)
    ...
    setCurrentTimeFromTimeLine (event) {
      ...
      const timePointerPosition = ((event.pageX - this.$refs.timeLine.offsetLeft) / this.$refs.timeLine.offsetWidth).toFixed(2)

      this.currentMovieElement.currentTime = (this.durations[this.currentMovie] * timePointerPosition).toFixed(2)
    },
    setControls () {
  ...
  filters: {
    timeFilter (value) {
      return Math.floor(value) < 10 ? '0' + Math.floor(value) : Math.floor(value)
    }
  },
  mounted () {
    for (let i = 0; i < 5; i++) {
